...
class StockMongo(object):
    ...
    def pop(self):
        ...
        record = self.db.find_one_and_update(
            {'status':self.OUTSIANDING},#取出待爬取的股票代码
    ...
    def status_setting(self):
        ...
        for i in record:
            id = i["_id"]
            self.db.update_one({'_id': id}, {'$set': {'status': self.OUTSIANDING}})  # 该状态为1，重新测试
            print('股票', id, '状态更改成功')
    ...
    def push_stock_comment(self,comment_id,symbol,comment,user_id,user,title,):
        ...
        try:
            self.db.update_one({'评论ID':comment_id},{'$set': {'评论ID':comment_id,'股票代码':symbol,'评论内容':comment,'评论者ID':user_id,'评论标题':title}},True)
            print('股票',symbol,'的一页评论内容插入成功')
        except:
            print('评论保存失败', comment_id, symbol)
            traceback.print_exc()

    def complete(self, symbol):
        self.db.update_one({'_id': symbol}, {'$set': {'status': self.COMPLETE}})
